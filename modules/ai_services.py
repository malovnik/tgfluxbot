...
async def generate_prompt(text: str, user_id: int = None) -> Optional[str]:
    ...
    Returns:
        Optional[str]: Промпт на английском языке с префиксом ВАШ_ПРЕФИКС_ФЛАКС или None в случае ошибки
    """
    try:
        ...
        # Добавляем префикс ВАШ_ПРЕФИКС_ФЛАКС к промпту, если он еще не добавлен
        if not prompt.lower().startswith("ВАШ_ПРЕФИКС_ФЛАКС"):
            prompt = f"ВАШ_ПРЕФИКС_ФЛАКС {prompt}"
...
async def analyze_image(image_description: str, user_id: int = None) -> Optional[str]:
    ...
    Returns:
        Optional[str]: Промпт для создания похожего изображения с человеком 'ВАШ_ПРЕФИКС_ФЛАКС' или None в случае ошибки
    """
    try:
        # Получаем настройки пользователя, если предоставлен ID
        model = DEFAULT_OPENAI_MODEL
